...
function jsFunction(value)
{
	...
	var groupKey = "<c:out value="${countries.isoCode}"/>";

	if (groupKey == value){
		
		<c:forEach var="citys" items="${countries.citys}">
		
		 
		
		
		var val = "<c:out value="${citys.id}"/>";
		var name = "<c:out value="${citys.name}"/>";
		
		
		
		$("#citysId").append("<option value="+val+">"+name+"</option>");
			            ...
			            <div class="controls">
 	 		            ...
 	 		            <div class="control-group">
	                        ...
	                        <div class="controls"> 				       							
	       							...
	       							<form:select cssClass="billing-country-list" path="billing.country.isoCode" onchange="jsFunction(this.value)">
		  									
		  									
		  									<c:forEach var="countries" items="${countries}">
  <option value="${countries.isoCode}"  <c:if test="${countries.id == 1}"> selected </c:if>  >${countries.name} </option>
</c:forEach>
				 		...
				 		<%-- 	<form:input  cssClass="input-large highlight"  maxlength="100" path="billing.city"/> --%>
				 			    ...
				 			    <div class="controls"> 				       							
	       							<form:select cssClass="billing-country-list" path="billing.city" id="citysId">


						<c:forEach var="countries" items="${countries}">
							<c:if test="${countries.id == 1}">
								<c:forEach var="citys" items="${countries.citys}">
									...
									<option value="${citys.id}">${citys.name}</option>
                                 	...
                                 	<span class="help-inline"><form:errors path="billing.city" cssClass="error" /></span>
	                        </div>  
